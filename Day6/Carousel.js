...
function hideImage(index) {
    images[index].style.animation = ''; // 애니메이션 초기화
    images[index].classList.remove('active');
    images[index].style.opacity = '0'; // 이미지 투명도를 0으로 설정
    dots[index].classList.remove('active'); // 해당 dot을 비활성화
}

function showImage(index) {
    images[index].style.animation = 'slide 0.5s forwards'; // 애니메이션 설정
    images[index].classList.add('active');
    images[index].style.opacity = '1'; // 이미지 투명도를 1로 설정
